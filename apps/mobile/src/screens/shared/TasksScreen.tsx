...
/**
 * Which task audiences a role belongs to.
 *
 * `campus_cartel` reaches everyone in the community; `students` and `ambassadors`
 * are mutually exclusive. Mirrored by the tasks_select policy in migration 023 —
 * this is the convenience filter, the policy is the enforcement.
 */
function getGroupsForRole(role: string | undefined): TaskTargetGroup[] {
  switch (role) {
    ...
    default:
      // Unknown role belongs to no audience — don't fall through to student tasks.
      return [];
  }
}
...
export default function TasksScreen() {
  ...
  const handleSubmit = useCallback(
    async (task: Task) => {
      // Both are proof the admin reviews before releasing coins, so neither is optional.
      if (!submissionNote.trim()) {
        Alert.alert('Description required', 'Please describe what you did for this task.');
        return;
      }
      if (!submissionImageUri) {
        Alert.alert('Photo required', 'Please attach a photo or screenshot as proof.');
        return;
      }
      if (!user?.id) return;
      ...
      setSubmitting(true);
      try {
        const imageUrl = await uploadEventImage(submissionImageUri, user.id);
        if (!imageUrl) {
          // Previously this fell through and submitted with no photo at all, leaving
          // the admin to approve a coin payout on the description alone.
          showError(null, {
            context: 'Your photo could not be uploaded. Please check your connection and try again.',
          });
          return;
        }
                    ...
                    <View style={styles.expandedArea}>
                      ...
                      <View style={styles.submissionForm}>
                        ...
                        <Text style={styles.submissionHint}>
                          A description and a photo are both required — the admin reviews
                          them before releasing your coins.
                        </Text>
                        ...
                        <TouchableOpacity
                          style={[
                            ...
                            !submissionImageUri && styles.imagePickerBtnRequired,
                          ]}
                          onPress={pickImage}
                          activeOpacity={0.8}
                        >
...
const styles = StyleSheet.create({
  ...
  /** Solid lime border until a photo is attached, so the missing proof is obvious. */
  imagePickerBtnRequired: {
    borderStyle: 'solid',
